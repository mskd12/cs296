...
int main(int argc, char** argv)
{
  ...
  //this is the timer object to time the iterations
  b2Timer timer;
  timer.Reset();
  //start time
  double time_start = timer.GetMilliseconds();
  double step_counter = 0, col_counter = 0, vel_counter = 0, pos_counter = 0;

  int no_iterations = atoi(argv[1]);
  for (int  i = 0; i < no_iterations; i++) {
    (test->get_world())->Step(0.001, settings.velocity_iterations, settings.position_iterations);
    step_counter += ((test->get_world())->GetProfile()).step;
    col_counter += ((test->get_world())->GetProfile()).collide;
    vel_counter += ((test->get_world())->GetProfile()).solveVelocity;
    pos_counter += ((test->get_world())->GetProfile()).solvePosition;
  }
  //end time
  double time_end = timer.GetMilliseconds();

  //counter = (test->get_profile()).step;

  /*Number of Iterations: 150
          Average time per step is 4.9521 ms
          Average time for collisions is 2.6754 ms
          Average time for velocity updates is 0.8979 ms
          Average time for position updates is 0.01870 ms

          Total loop time is 607.0126 ms*/

  printf("Number of Iterations : %i\n", no_iterations);  

  printf("Average time per step is %f ms\n", step_counter / no_iterations);

  printf("Average time for collisions is %f ms\n", col_counter / no_iterations);

  printf("Average time for velocity updates is %f ms\n", vel_counter / no_iterations);

  printf("Average time for position updates is %f ms\n\n", pos_counter / no_iterations);
